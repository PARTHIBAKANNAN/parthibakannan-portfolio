...
export function Nav() {
  ...
  return (
    <nav style={{
      ...
      // Fully opaque (not the translucent scrolled tint) whenever the mobile drawer is
      // open — the drawer sits over page content and can't rely on backdrop-filter blur
      // being supported to stay legible.
      ...
      <div className="container" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="hide-mobile" style={{ display: "flex", alignItems: "center", gap: 4 }}>
          {links.map(l => {
            ...
            const onEnter = e => { e.currentTarget.style.color = t.ink; e.currentTarget.style.background = "var(--chip-bg)"; };
      ...
      {menuOpen && (
        <div id="mobile-menu" className="show-mobile" style={{
          ...
          <div className="container" style={{ display: "flex", flexDirection: "column", gap: 2, paddingTop: 14, paddingBottom: 6 }}>
            {links.map(l => {
              const sharedStyle = { padding: "12px 6px", fontSize: 15, color: t.ink, fontWeight: 600, borderRadius: 8, background: "transparent", border: "none", fontFamily: "inherit", cursor: "pointer", textAlign: "left" };
              return l.onClick
                ? <button key={l.label} type="button" onClick={l.onClick} style={sharedStyle}>{l.label}</button>
                : <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} style={sharedStyle}>{l.label}</a>;
            })}
